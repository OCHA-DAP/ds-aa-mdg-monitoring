...
def plot_rainfall(df: pd.DataFrame):
    ...
    imerg_pivot.plot(
        kind="bar", stacked=True, figsize=(10, 6), ax=ax, cmap="Paired"
    )
    ...
    ax.set_ylabel(
        f"Total 3-day rainfall, average over region,\n"
        f"centered on {middle_date} (mm)"
    )
    ax.set_title(f"Total 3-day rainfall around {middle_date}")
